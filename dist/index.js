...
class PixiRenderer {
    ...
    mount(container) {
        ...
        const supportsAsyncInit = typeof PIXI__namespace.Application?.prototype?.init === 'function';
        ...
        this._app = new PIXI__namespace.Application(appOptions);
        container.appendChild(this._getView());
        this._setupScene(container);
    }
    _setupScene(container) {
        this._rootStage = new PIXI__namespace.Container();
        this._app.stage.addChild(this._rootStage);
        this._gridLayer = new PIXI__namespace.Graphics();
        this._rootStage.addChild(this._gridLayer);
        this._guideLayer = new PIXI__namespace.Graphics();
        this._selectionLayer = new PIXI__namespace.Graphics();
    ...
    _drawDocBackground(doc) {
        if (!this._docBg) {
            this._docBg = new PIXI__namespace.Graphics();
    ...
    _createDisplayObject(el, parentContainer, doc) {
        ...
        if (el.type === 'text') {
            const textObj = new PIXI__namespace.Text(el.content, {
        ...
        if (el.type === 'image' && el.src) {
            PIXI__namespace.Texture.fromURL(el.src).then(tex => {
                const sprite = new PIXI__namespace.Sprite(tex);
                sprite.width = w;
                sprite.height = h;
                gfx.mask = (() => {
                    const m = new PIXI__namespace.Graphics();
        ...
        // Box children
        if (el.type === 'box') {
            const childContainer = new PIXI__namespace.Container();
        ...
        container.hitArea = new PIXI__namespace.Rectangle(0, 0, w, h);
    ...
    _getParentContainer(el) {
        ...
        if (parentObj) {
            const childContainer = parentObj.children.find(c => c instanceof PIXI__namespace.Container && c !== parentObj.children[0]);
